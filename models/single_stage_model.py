...
class SingleStageModel(object):
    def __init__(self, params, dist_model=False, boundary_output= True, boundary_with_shared = False):
        if boundary_output:
            self.model = backbone.__dict__[params['backbone_arch_boundary']](**params['backbone_param'])
        else:
            self.model = backbone.__dict__[params['backbone_arch_noboundary']](**params['backbone_param'])
