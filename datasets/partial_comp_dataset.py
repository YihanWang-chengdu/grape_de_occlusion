...
class PartialCompDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # eraser1 = eraser.copy()
        # modal1[modal1!=0]=255
        # eraser1[eraser1 != 0] = 255
        # cv2.imwrite('./modal1.png',modal1)
        # cv2.imwrite('./eraser1.png', eraser1)
        eraser = self.eraser_setter(modal, eraser) # uint8 {0, 1}     # eraser_setter是将eraser做一定量的偏移和 旋转  以获得更好的遮挡

        # eraser_setter1 = eraser.copy()
        # eraser_setter1[eraser_setter1 != 0] = 255
        # cv2.imwrite('./eraser_setter.png',eraser_setter1)
        ...
        # erased_modal[erased_modal != 0] = 255
        # cv2.imwrite('./erased_modal.png', erased_modal)
        ...
        return rgb, erased_modal_tensor, eraser_tensor, target                      #彩色图，被遮挡图，遮挡图，目标掩码
